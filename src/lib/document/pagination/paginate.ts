import type { ResolvedContract } from '../resolved-types';
...
export function paginateDocument(
	...
	for (const [blockIndex, block] of document.blocks.entries()) {
		...
		if (block.type === 'table') {
			const table: PageFragment = { ...block, blockKey };
			if (
				!measurement.fits([...currentPage().fragments, table], pages.length - 1) &&
				currentPage().fragments.length > 0
			) {
				newPage();
			}

			if (!measurement.fits([table], pages.length - 1)) {
				throw new Error('A table is taller than a page.');
			}
