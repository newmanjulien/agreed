...
} from './dom-text-index.ts';
import type { IndexedTextBlock, TextPoint } from './dom-text-index.ts';
import type { TextAnchor, TextAnchorSegment } from './types.ts';
...
const SELECTABLE_BLOCK_SELECTOR =
	'.contract-heading[data-block-id], .contract-paragraph[data-block-id]';
...
export function createTextAnchor(root: HTMLElement, range: Range): TextAnchor | null {
	const selectableBlockIds = new Set(
		Array.from(root.querySelectorAll<HTMLElement>(SELECTABLE_BLOCK_SELECTOR), (element) =>
	...
	for (const block of indexDocumentText(root)) {
		...
		if (!selectableBlockIds.has(block.id)) return null;
		segments.push({
			blockId: block.id,
			...interval,
