...
>
	...
	<div class="relative">
		<select
			class="h-12 w-full cursor-pointer appearance-none rounded-2xl border border-line bg-surface px-3 pr-10 text-[14.5px] text-ink shadow-none transition-colors hover:border-muted focus-within:border-accent focus-within:outline-2 focus-within:outline-offset-1 focus-within:outline-accent/18 disabled:cursor-wait disabled:bg-canvas disabled:text-muted motion-reduce:transition-none"
	...
	{#if isApplied}
		<p
			id={appliedMessageId}
			class="mt-2.5 mb-0 text-[13.5px] leading-[1.45] text-muted"
			role="status"
		>
	...
	{#if errorMessage}
		<p id={errorMessageId} class="mt-2.5 mb-0 text-[11.5px] leading-[1.35] text-danger" role="alert">
			{errorMessage}
		</p>
	{/if}
</WidgetFrame>
