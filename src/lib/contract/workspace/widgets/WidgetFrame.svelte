...
<aside
	class="widget-frame w-full rounded-2xl border border-line bg-surface p-3.5 text-ink shadow-none"
	aria-label={label}
>
	<header class="widget-frame-header" class:has-intro={intro !== undefined}>
		{#if intro !== undefined}
			<p id={introId} class="m-0 min-w-0 text-[14.5px] leading-[1.45] text-muted">{intro}</p>
		{/if}
