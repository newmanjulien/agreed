...
<WidgetFrame label={title} intro={definition.intro} {onDismiss}>
	<div class="flex flex-col gap-2">
		{#each definition.items as item}
			...
			<div>
				<button
					class="w-full cursor-pointer rounded-panel border-0 bg-canvas px-3.5 py-3 text-left text-[15px] leading-[1.3] text-ink hover:bg-hover-subtle focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-accent"
					type="button"
